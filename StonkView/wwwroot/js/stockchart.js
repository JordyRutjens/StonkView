...
StockChart.prototype.parse = function (response) {
    try {
        var data = JSON.parse(response)["Time Series (Daily)"];
        ...
        for (var prop in data) {
            results.date.push(prop);
            results.close.push(parseFloat(data[prop]["4. close"]));
            results.volume.push(parseFloat(data[prop]["5. adjusted close"]));

            results.open.push(parseFloat(data[prop]["1. open"]));
            results.high.push(parseFloat(data[prop]["2. high"]));
            results.low.push(parseFloat(data[prop]["3. low"]));
        }
        results.date.forEach(function (value, index, array) {
            array[index] = Date.parse(value);
        });

        results["date"].reverse();
        results["close"].reverse();
        results["volume"].reverse();
        results["open"].reverse();
        results["high"].reverse();
        results["low"].reverse();
...
StockChart.prototype.svg = function (tagname) {
    try {
...
StockChart.prototype.attrNS = function (ns, element, attributes) {
    try {
...
StockChart.prototype.attr = function (element, attributes) {
    try {
...
StockChart.prototype.css = function (element, attributes) {
    try {
...
StockChart.prototype.image = function (index) {
    try {
        switch (index) {
            case 1:
...
StockChart.prototype.closest = function (number, array) {
    try {
        var difference;
        var index = 0;
...
StockChart.prototype.init = function (x_i, x_ii) {
    try {
        var pointer = this;
        if (x_i < 0) x_i = 0;
        ...
        svg.addEventListener("mousemove", function (event) {
            ...
            if (n) {
                ...
                pointer8.textContent = "Volume: " + parseFloat(pointer.cache.volume[i + x_i]).toFixed(0);
            };
        });
